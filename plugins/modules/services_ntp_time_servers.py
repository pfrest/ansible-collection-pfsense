...
DOCUMENTATION = '''
module: services_ntp_time_servers
description:
- Manage all NTP Time Servers.
short_description: Manage all NTP Time Servers.
options:
  api_host:
    type: str
    required: true
  ...
  objects:
    ...
    suboptions:
      ...
      type:
        ...
        - pool
        - peer
        description: 'The type of this timeserver. Use `server` is `timeserver` is
          a standalone NTP server, use `pool` if `timeserver` represents an NTP pool,
