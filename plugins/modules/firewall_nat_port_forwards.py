...
###############################################################
"""An Ansible module for interacting with /api/v2/firewall/nat/port_forwards."""
...
DOCUMENTATION = r"""
module: firewall_nat_port_forwards
description:
    ...
    suboptions:
      ...
      source:
        ...
        description: 'The source address this port forward rule applies to. Valid
          ...
          an existing alias, `any`, `(self)`, `l2tp`, `pppoe`. The context of this
          address can be inverted by prefixing the value with `!`. For interface values,
          the `:ip` modifier can be appended to the value to use the interface''s
      ...
      target:
        ...
        description: 'The IP address or alias of the internal host to forward matching
          ...
          interface values, the `:ip` modifier can be appended to the value to use
      ...
      natreflection:
        ...
        - disable
        - purenat
        description: The NAT reflection mode to use for traffic matching this port
          forward rule. Set to `null` to use the system default.
      associated_rule_id:
        required: false
        type: str
        default: ''
        ...
        description: The associated firewall rule mode. Use an empty string to require
          ...
          forward rule. Use `new` to create a new associated firewall rule to pass
          traffic matching this port forward rule. Use `pass` to automatically pass
          traffic matching this port forward rule without the need for a firewall
          rule. Otherwise, you can specify the `associated_rule_id` of an existing
          firewall rule to associate with this port forward rule.
...
EXAMPLES = """
- name: Manage all Port Forwards
  pfrest.pfsense.firewall_nat_port_forwards:
...
def run_module():
    """Runs this collection module against /api/v2/firewall/nat/port_forwards."""
