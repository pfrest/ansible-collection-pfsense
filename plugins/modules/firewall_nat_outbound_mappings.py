...
short_description: Manage all Outbound NAT Mappings.
options:
  api_host:
    type: str
    required: true
  ...
  objects:
    ...
    suboptions:
      ...
      source_port:
        ...
        default: null
        choices: []
        description: 'The source port this rule should match. Valid options are: a
          TCP/UDP port number, a TCP/UDP port range separated by `:`, an existing
      ...
      poolopts:
        ...
        - source-hash
        - bitmask
        description: The pool option used to load balance external IP mapping when
          `target` is set to a subnet or alias of many addresses. Set to `null` to
