...
from .embedded_schema import SCHEMA_DICT
...
class NativeSchema:
    ...
    def get_endpoint_schema(self, endpoint: str) -> dict:
        ...
        if endpoint not in self.full_schema.get("endpoints", {}):
            raise LookupError(
                f"Could not find schema for endpoint with URL {endpoint}. This likely "
                f"means the version of pfSense-pkg-RESTAPI running on the target host "
                f"does not support this endpoint."
            )
